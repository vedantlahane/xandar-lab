...
export function ProblemDrawer({
    ...
    const [loadingDiscussions, setLoadingDiscussions] = useState<Record<string, boolean>>({});
    ...
    const fetchDiscussions = async (attemptId: string) => {
        ...
        try {
            const res = await fetch(`/api/attempts/discussions?attemptId=${attemptId}`, {
                credentials: 'include',
            });
            if (res.ok) {
                const data = await res.json();
                setDiscussions(prev => ({ ...prev, [attemptId]: data.discussions || [] }));
            }
        } catch (error) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setAttempts(prev => prev.map(a =>
                    a._id === attemptId ? data.attempt : a
                ));
            }
        } catch (error) {
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex gap-1">
                            {['details', 'attempts'].map((tab) => (
                                ...
                                >
                                    {tab === 'details' ? 'Details' : `Attempts (${attempts.length})`}
                                </button>
                            ))}
                        </div>
                    </div>
                        ...
                        {activeTab === 'details' ? (
                            ...
                            >
                                <div className="space-y-3">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        ...
                                        {problem.tags?.map((tag) => (
                                            <Badge key={tag} subtle>{tag}</Badge>
                                        ))}
                                    </div>
                                </div>
                                ...
                                <div className="flex gap-3">
                                    ...
                                    <Button
                                        variant="outline"
                                        onClick={() => { setActiveTab('attempts'); setShowNewAttempt(true); }}
                                    >
                                ...
                                {attempts.length > 0 && (
                                    <div className="pt-4 border-t border-border/50">
                                        <p className="text-xs text-muted-foreground">
                                            You have {attempts.length} attempt{attempts.length !== 1 ? 's' : ''} on this problem.
                                            <button
                            ...
                            >
                                        ...
                                        >
                                            ...
                                            <div className="flex justify-end gap-2">
                                                ...
                                                <Button
                                                    ...
                                                    disabled={!newAttemptContent.trim() || submittingAttempt}
                                                >
                                                    {submittingAttempt ? (
                                ...
                                {loadingAttempts ? (
                                    ...
                                    <div className="text-center py-8 text-muted-foreground">
                                        ...
                                        <p className="text-xs">Start your first attempt to track your understanding</p>
                                    </div>
                                ) : (
                                    <div className="space-y-3">
                                                ...
                                                <div className="flex items-start justify-between">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="text-xs font-medium">
                                                            {attempt.status === 'resolved' ? 'Resolved' : 'Attempting'}
                                                        </span>
                                                    ...
                                                    <div className="flex items-center gap-1">
                                                        {attempt.status === 'attempting' && (
                                                            <Button
                                                                ...
                                                                onClick={() => handleResolveAttempt(attempt._id)}
                                                            ...
                                                            {loadingDiscussions[attempt._id] ? (
                                                                ...
                                                                <>
                                                                    {discussions[attempt._id]?.map((d) => (
                                                                        <div key={d._id} className="flex gap-2 text-xs">
                                                                            ...
                                                                            <div className="flex-1">
                                                                                ...
                                                                                <p className="mt-0.5 text-muted-foreground">{d.content}</p>
                                                                            </div>
                                                                        </div>
                                                                    ))}
                                                                    ...
                                                                    <div className="flex gap-2">
                                                                        ...
                                                                        <Button
                                                                            size="icon"
                                                                            className="h-6 w-6"
                                                                            onClick={() => handleAddComment(attempt._id)}
                                                                            disabled={!newComment[attempt._id]?.trim() || submittingComment[attempt._id]}
                                                                        >

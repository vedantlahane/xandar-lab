...
import { Send, Lightbulb, Clock, X, HelpCircle, ArrowRight, Check } from "lucide-react";
...
export function ActiveInterview({ config, onEnd }: ActiveInterviewProps) {
  ...
  const totalSeconds = 45 * 60; // 45 mins
  ...
  return (
    <div className="h-full flex flex-col pt-6 pb-6 px-12 max-w-4xl mx-auto w-full">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-border/40">
        <div>
          <h2 className="text-sm font-semibold">{config.style} Style · {config.difficulty}</h2>
        ...
        <div className="flex items-center gap-6">
          ...
          <Button variant="ghost" size="sm" onClick={onEnd} className="text-muted-foreground hover:text-destructive hover:bg-destructive/10 -mr-2">
      ...
      {/* Input Area */}
      <div className="pt-4 border-t border-border/40 pb-20"> {/* extra pb for mobile spacing */}
        ...
        <div className="flex gap-3">
          <Button variant="outline" size="sm" className="text-xs h-8 text-muted-foreground">
            Ask to clarify
          </Button>
          {phase === "Understanding" ? (
             <Button variant="secondary" size="sm" className="text-xs h-8" onClick={() => setPhase("Approach")}>
               I understand, let's discuss approach <ArrowRight className="h-3 w-3 ml-1" />
             </Button>
          ) : phase === "Approach" ? (
             <Button variant="secondary" size="sm" className="text-xs h-8" onClick={() => setPhase("Code")}>
               Ready to code <ArrowRight className="h-3 w-3 ml-1" />
             </Button>
          ) : phase === "Code" ? (
             <Button variant="secondary" size="sm" className="text-xs h-8" onClick={() => setPhase("Review")}>
               I submitted on LeetCode <Check className="h-3 w-3 ml-1" />
             </Button>
          ) : (
            <Button variant="default" size="sm" className="text-xs h-8" onClick={onEnd}>
               End Interview
             </Button>
          )}
        </div>
